...
var Empty = (function() {
  ...
  _setupSocket = function() {
    ...
    this._nameSpaceIo.on('connection', function(socket) {
      ...
      socket.on('_binary', function(data, callback) {
        console.log('[Controller] Empty -> Receive _binary', data.blob);
        socket.emit('_binary', { image: true, buffer: data.blob });
        // var fs = require('fs');
        // fs.readFile('test.png', function(err, buf){
        //   console.log(buf);
        //   // it's possible to embed binary data
        //   // within arbitrarily-complex objects
        //   socket.emit('_binary', { image: true, buffer: buf });
        // });
      });
    });
